<%@ page import='java.util.*,com.doublechaintech.lsc.*,com.doublechaintech.lsc.location.Location'%>
...
<div class="row">
	...
	<% Location result = (Location)request.getAttribute("result");  %>
			<li><a data-toggle="tab" href="#transportTaskListAsSource" class="disabled"> ${userContext.localeMap['transport_task']}</a></li>
			<li><a data-toggle="tab" href="#transportTaskListAsDestination" class="disabled"> ${userContext.localeMap['transport_task']}</a></li>
...
<sky:include page="com/doublechaintech/lsc/location/Location$Summary.jsp" />
...
<sky:include page="com/doublechaintech/lsc/platform/Platform$Info.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["transportTaskListAsSource"] or ignoreListAccessControl}'>
		<c:set var="transportTaskList" value="${result.transportTaskListAsSource}" scope="request"/>
		<c:set var="transportTaskListName" value="transportTaskListAsSource" scope="request"/>
		<div id="transportTaskListAsSource" class="tab-pane fade sublist" refer-name="source">
			<sky:include page="com/doublechaintech/lsc/transporttask/TransportTask$List.jsp"
					referName="source"/>
		</div>
	</c:if>
	<c:if test='${not empty userContext.accessTokens["transportTaskListAsDestination"] or ignoreListAccessControl}'>
		<c:set var="transportTaskList" value="${result.transportTaskListAsDestination}" scope="request"/>
		<c:set var="transportTaskListName" value="transportTaskListAsDestination" scope="request"/>
		<div id="transportTaskListAsDestination" class="tab-pane fade sublist" refer-name="destination">
			<sky:include page="com/doublechaintech/lsc/transporttask/TransportTask$List.jsp"
					referName="destination"/>
		</div>
	</c:if>
