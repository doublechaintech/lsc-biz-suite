...
import styles from './TransportProject.preference.less'
...
const internalImageListOf = (transportProject) =>{
...
const internalSettingListOf = (transportProject) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = TransportProjectService[candidateServiceName] 
...
const executeTrans = (transportProject,targetComponent) =>{
...
const buildTransferModal = (transportProject,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(transportProject,targetComponent)}
...
const internalRenderExtraHeader = (transportProject) =>{
	return null
}
const internalRenderExtraFooter = (transportProject) =>{
	return null
}
...
const renderPermissionSetting = transportProject => {
  const {TransportProjectBase} = GlobalComponents
  return <PermissionSetting targetObject={transportProject}  targetObjectMeta={TransportProjectBase}/>


}


class TransportProjectPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, transportItemCount, transportTaskCount } = this.props.transportProject
...
}))(Form.create()(TransportProjectPreference))
