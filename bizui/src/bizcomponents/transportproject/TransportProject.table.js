...
import styles from './TransportProject.table.less'
...
class TransportProjectTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = TransportProjectBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/transportProject/${record.id}/dashboard`}>{'查看'}</Link>}
...
export default TransportProjectTable
