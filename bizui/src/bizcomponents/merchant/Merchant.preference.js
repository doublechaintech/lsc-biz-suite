...
import styles from './Merchant.preference.less'
...
const internalImageListOf = (merchant) =>{
...
const internalSettingListOf = (merchant) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MerchantService[candidateServiceName] 
...
const executeTrans = (merchant,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_merchant/doJob",payload: payload})
...
const buildTransferModal = (merchant,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(merchant,targetComponent)}
...
const internalRenderExtraHeader = (merchant) =>{
	return null
}
const internalRenderExtraFooter = (merchant) =>{
	return null
}
...
<Description term="类型">{merchant.type==null?"未分配":merchant.type.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"类型","merchantType","requestCandidateType",
...
const renderPermissionSetting = merchant => {
  const {MerchantBase} = GlobalComponents
  return <PermissionSetting targetObject={merchant}  targetObjectMeta={MerchantBase}/>


}


class MerchantPreference extends Component {
...
}))(Form.create()(MerchantPreference))
