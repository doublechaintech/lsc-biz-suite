...
import styles from './MerchantAccount.preference.less'
...
const internalImageListOf = (merchantAccount) =>{
...
const internalSettingListOf = (merchantAccount) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MerchantAccountService[candidateServiceName] 
...
const executeTrans = (merchantAccount,targetComponent) =>{
...
const buildTransferModal = (merchantAccount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(merchantAccount,targetComponent)}
...
const internalRenderExtraHeader = (merchantAccount) =>{
	return null
}
const internalRenderExtraFooter = (merchantAccount) =>{
	return null
}
...
<Description term="Merchant">{merchantAccount.merchant==null?"未分配":merchantAccount.merchant.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Merchant","merchant","requestCandidateMerchant",
	      "transferToAnotherMerchant","anotherMerchantId",merchantAccount.merchant?merchantAccount.merchant.id:"")} 
...
const renderPermissionSetting = merchantAccount => {
  const {MerchantAccountBase} = GlobalComponents
  return <PermissionSetting targetObject={merchantAccount}  targetObjectMeta={MerchantAccountBase}/>


}


class MerchantAccountPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, transactionCount } = this.props.merchantAccount
    const cardsData = {cardsName:"Merchant Account",cardsFor: "merchantAccount",cardsSource: this.props.merchantAccount,
  		subItems: [
...
}))(Form.create()(MerchantAccountPreference))
