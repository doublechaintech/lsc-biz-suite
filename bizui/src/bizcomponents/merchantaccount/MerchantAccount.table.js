...
import styles from './MerchantAccount.table.less'
...
class MerchantAccountTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = MerchantAccountBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/merchantAccount/${record.id}/dashboard`}>{'查看'}</Link>}
...
export default MerchantAccountTable
