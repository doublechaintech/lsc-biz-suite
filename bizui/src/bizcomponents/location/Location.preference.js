...
import styles from './Location.preference.less'
...
const internalImageListOf = (location) =>{
...
const internalSettingListOf = (location) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = LocationService[candidateServiceName] 
...
const executeTrans = (location,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_location/doJob",payload: payload})
...
const buildTransferModal = (location,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(location,targetComponent)}
...
const internalRenderExtraHeader = (location) =>{
	return null
}
const internalRenderExtraFooter = (location) =>{
	return null
}
...
class LocationPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, transportTaskAsSourceCount, transportTaskAsDestinationCount } = this.props.location
    const cardsData = {cardsName:"位置",cardsFor: "location",cardsSource: this.props.location,
  		subItems: [
...
}))(Form.create()(LocationPreference))
