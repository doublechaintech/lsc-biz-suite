...
import styles from './MerchantType.preference.less'
...
const internalImageListOf = (merchantType) =>{
...
const internalSettingListOf = (merchantType) =>{
...
const internalLargeTextOf = (merchantType) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MerchantTypeService[candidateServiceName] 
...
const executeTrans = (merchantType,targetComponent) =>{
...
const buildTransferModal = (merchantType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(merchantType,targetComponent)}
...
const internalRenderExtraHeader = (merchantType) =>{
	return null
}
const internalRenderExtraFooter = (merchantType) =>{
	return null
}
...
const internalSummaryOf = (merchantType,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{merchantType.id}</Description> 
<Description term="名称">{merchantType.name}</Description> 
	
        {buildTransferModal(merchantType,targetComponent)}
      </DescriptionList>
	)
...
const renderPermissionSetting = merchantType => {
  const {MerchantTypeBase} = GlobalComponents
  return <PermissionSetting targetObject={merchantType}  targetObjectMeta={MerchantTypeBase}/>


}


class MerchantTypePreference extends Component {
...
}))(Form.create()(MerchantTypePreference))
