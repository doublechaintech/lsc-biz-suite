...
import styles from './Transaction.preference.less'
...
const internalImageListOf = (transaction) =>{
...
const internalSettingListOf = (transaction) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = TransactionService[candidateServiceName] 
...
const executeTrans = (transaction,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_transaction/doJob",payload: payload})
...
const buildTransferModal = (transaction,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(transaction,targetComponent)}
...
const internalRenderExtraHeader = (transaction) =>{
	return null
}
const internalRenderExtraFooter = (transaction) =>{
	return null
}
...
class TransactionPreference extends Component {
...
}))(Form.create()(TransactionPreference))
