...
import styles from './TransportTaskStatus.table.less'
...
class TransportTaskStatusTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = TransportTaskStatusBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/transportTaskStatus/${record.id}/dashboard`}>{'查看'}</Link>}
...
export default TransportTaskStatusTable
