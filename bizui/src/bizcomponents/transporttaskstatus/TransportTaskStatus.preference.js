...
import styles from './TransportTaskStatus.preference.less'
...
const internalImageListOf = (transportTaskStatus) =>{
...
const internalSettingListOf = (transportTaskStatus) =>{
...
const internalLargeTextOf = (transportTaskStatus) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = TransportTaskStatusService[candidateServiceName] 
...
const executeTrans = (transportTaskStatus,targetComponent) =>{
...
const buildTransferModal = (transportTaskStatus,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(transportTaskStatus,targetComponent)}
...
const internalRenderExtraHeader = (transportTaskStatus) =>{
	return null
}
const internalRenderExtraFooter = (transportTaskStatus) =>{
	return null
}
...
const internalSummaryOf = (transportTaskStatus,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{transportTaskStatus.id}</Description> 
<Description term="名称">{transportTaskStatus.name}</Description> 
	
        {buildTransferModal(transportTaskStatus,targetComponent)}
      </DescriptionList>
	)
...
const renderPermissionSetting = transportTaskStatus => {
  const {TransportTaskStatusBase} = GlobalComponents
  return <PermissionSetting targetObject={transportTaskStatus}  targetObjectMeta={TransportTaskStatusBase}/>


}


class TransportTaskStatusPreference extends Component {
...
}))(Form.create()(TransportTaskStatusPreference))
