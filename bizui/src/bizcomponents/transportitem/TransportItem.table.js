...
import styles from './TransportItem.table.less'
...
class TransportItemTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = TransportItemBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/transportItem/${record.id}/dashboard`}>{'查看'}</Link>}
...
export default TransportItemTable
