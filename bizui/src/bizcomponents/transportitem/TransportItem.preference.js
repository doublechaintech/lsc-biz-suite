...
import styles from './TransportItem.preference.less'
...
const internalImageListOf = (transportItem) =>{
...
const internalSettingListOf = (transportItem) =>{
...
const internalLargeTextOf = (transportItem) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {TransportItemService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = TransportItemService[candidateServiceName] 
...
const executeTrans = (transportItem,targetComponent) =>{
...
const buildTransferModal = (transportItem,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(transportItem,targetComponent)}
...
const internalRenderExtraHeader = (transportItem) =>{
	return null
}
const internalRenderExtraFooter = (transportItem) =>{
	return null
}
...
const internalSummaryOf = (transportItem,targetComponent) =>{
...
const renderPermissionSetting = transportItem => {
  const {TransportItemBase} = GlobalComponents
  return <PermissionSetting targetObject={transportItem}  targetObjectMeta={TransportItemBase}/>


}


class TransportItemPreference extends Component {
...
export default connect(state => ({
  transportItem: state._transportItem,
}))(Form.create()(TransportItemPreference))
